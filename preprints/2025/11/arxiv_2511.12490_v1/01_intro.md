...
|  |  |  |  |
| --- | --- | --- | --- |
|  | BASEi,t=0.7×valuei,t+0.3×reversali,t\text{BASE}\_{i,t}=0.7\times\text{value}\_{i,t}+0.3\times\text{reversal}\_{i,t} |  | (1) |
...
|  |  |  |  |
| --- | --- | --- | --- |
|  | UpFractioni,t=163​∑k=163𝕀​[ri,t−k>0]\text{UpFraction}\_{i,t}=\frac{1}{63}\sum\_{k=1}^{63}\mathbb{I}[r\_{i,t-k}>0] |  | (2) |

defining stocks as being in drift regime when this fraction exceeds 60%:

|  |  |  |  |
| --- | --- | --- | --- |
|  | REGIMEi,t=𝕀​[UpFractioni,t>0.60]\text{REGIME}\_{i,t}=\mathbb{I}[\text{UpFraction}\_{i,t}>0.60] |  | (3) |

The Unicorn Edge combines the BASE signal with regime filter through simple multiplication:

|  |  |  |  |
| --- | --- | --- | --- |
|  | EDGEi,t=BASEi,t×REGIMEi,t\text{EDGE}\_{i,t}=\text{BASE}\_{i,t}\times\text{REGIME}\_{i,t} |  | (4) |
...
|  |  |  |  |
| --- | --- | --- | --- |
|  | s∗=min⁡(12%TrainingVol,15%|TrainingMaxDD|)s^{\*}=\min\left(\frac{12\%}{\text{TrainingVol}},\frac{15\%}{|\text{TrainingMaxDD}|}\right) |  | (5) |
...
Beyond static scaling, we implement dynamic kill-switch protection shutting down the strategy entirely if experiencing losses beyond historical norms, monitoring absolute drawdown from peak (30% threshold) and rolling 63-day performance (−10%-10\% threshold). The kill-switch prioritizes capital preservation over return maximization, with switches that cannot reset within evaluation periods preventing emotional re-entry attempts.

We employ rigorous walk-forward validation with true out-of-sample testing where all parameters are fixed based on historical data then applied to future periods without modification. Each evaluation consists of five-year training periods for parameter estimation followed by one-year test periods where we apply frozen strategies. We conduct three walk-forward windows: Window 1 covering post-financial crisis recovery (train 2005–2010, test 2010–2011), Window 2 representing stable markets (train 2010–2015, test 2015–2016), and Window 3 encompassing COVID disruption (train 2015–2020, test 2020–2021). Transaction costs of 0.6 basis points per unit traded reflect combined explicit and implicit costs for liquid large-cap stocks (Almgren and Chriss, [2001](https://arxiv.org/html/2511.12490v1#bib.bib10); Hendershott and Menkveld, [2014](https://arxiv.org/html/2511.12490v1#bib.bib13)), conservatively charged against subsequent returns.

![Refer to caption](plot1_three_panels_oos_real.png)


Figure 1: 
Out-of-sample equity curves for the Unicorn Edge strategy versus the S&P 500 equal-weight benchmark across three walk-forward test windows (2010–2011, 2015–2016, 2020–2021), shown on a logarithmic scale and rebased using prior-window ending wealth.
...
| 1 | 2005–2010 | 2010–2011 | 19.42 | 0.841 | 16.89 | 206.7% | 12.2% | −11.9%-11.9\% |
| 2 | 2010–2015 | 2015–2016 | 27.79 | 1.327 | 22.87 | 207.2% | 9.1% | −0.9%-0.9\% |
| 3 | 2015–2020 | 2020–2021 | 16.63 | 1.569 | 5.11 | 62.0% | 12.2% | −4.0%-4.0\% |

Our walk-forward validation produces high out-of-sample results far exceeding typical factor performance. The first window covering post-financial crisis recovery, as shown in Fig. [1](https://arxiv.org/html/2511.12490v1#S2.F1 "Figure 1 ‣ 2.2 Portfolio Implementation and Risk Management ‣ 2 Data and Methodology ‣ Discovery of a 13-Sharpe OOS Factor: Drift Regimes Unlock Hidden Cross-Sectional Predictability"), achieved a test period sharpe ratio of 16.89 with annualized returns of 206.7%, despite including European debt crisis turbulence. The second window in stable markets produced unusually high results with Sharpe ratio of 22.87 and remarkably shallow maximum drawdown of only −0.9%-0.9\%. The third window during COVID disruption, while showing lower performance, still achieved Sharpe ratio of 5.11, demonstrating robustness during unprecedented conditions.

![Refer to caption](unicorn_vs_sp500_kde_shaded_medians.png)


Figure 2: 
Shaded KDE distributions of daily returns for the Unicorn Edge strategy and the S&P 500 equal-weight benchmark, with vertical dashed lines marking each distribution’s median daily return.
...
| Sharpe Ratio (OOS) | 13.19 | 0.99 | 13.3×13.3\times |
| Total OOS Return (3 test years) | +10,938% | +478% | 22.9×22.9\times |
| Wealth Multiple (OOS) | 110.4×110.4\times | 5.78×5.78\times | 19.1×19.1\times |
| Max Drawdown | −11.9%-11.9\% | −18.7%-18.7\% | 0.64×0.64\times |
| Winning Days | 67% | 54% | 1.24×1.24\times |
| Best Day | +4.2%+4.2\% | +9.1%+9.1\% | 0.46×0.46\times |
| Worst Day | −3.1%-3.1\% | −8.3%-8.3\% | 0.37×0.37\times |
| Skewness | 0.42 | −0.31-0.31 | N/A |
| Correlation | 0.08 | 1.00 | N/A |

Note. Benchmark performance is computed only over the three one-year OOS test
windows (2010–2011, 2015–2016, 2020–2021), which coincide with exceptionally strong
...
Concatenating all test periods provides definitive out-of-sample assessment with combined Sharpe ratio of 13.19, annualized returns of 158.6%, volatility of 12.0%, and maximum drawdown of −11.9%-11.9\%—performance approximately 13 times better than market benchmarks achieved entirely out-of-sample with frozen parameters. The strategy produces positive returns on 67% of days with favorable skewness and minimal correlation to benchmarks.
...
Across the full out-of-sample period, the Unicorn Edge exhibits a median daily return of 0.63%, substantially higher than the S&P 500 equal-weight benchmark’s median of 0.11%.
This shift in the center of the distribution, together with the significantly thinner tails shown in Fig. [2](https://arxiv.org/html/2511.12490v1#S3.F2 "Figure 2 ‣ 3.1 Out-of-Sample Performance ‣ 3 Results ‣ Discovery of a 13-Sharpe OOS Factor: Drift Regimes Unlock Hidden Cross-Sectional Predictability"), indicates both higher day-to-day efficiency and improved return stability at the micro level—properties atypical for short-horizon cross-sectional strategies.

Table 3: Cumulative Wealth Evolution ($1M Initial)

| Period End | Unicorn Edge | Benchmark | Outperformance |
| --- | --- | --- | --- |
| Year 1 (2011) | $3,067,000 | $1,793,000 | 1.71×1.71\times |
| Year 2 (2016) | $9,415,000 | $3,220,000 | 2.92×2.92\times |
| Year 3 (2021) | $110,376,000 | $5,779,000 | 19.10×19.10\times |
...
| Parameter | Base Case | −30%-30\% | −15%-15\% | +15%+15\% | +30%+30\% | Range |
| --- | --- | --- | --- | --- | --- | --- |
| Drift Window (WW) | 13.2 (63d) | 8.7 (44d) | 11.1 (54d) | 11.8 (72d) | 9.2 (82d) | 8.7–13.2 |
| Up Threshold (θ\theta) | 13.2 (0.60) | 3.2 (0.42) | 9.8 (0.51) | 10.4 (0.69) | 4.1 (0.78) | 3.2–13.2 |
| Value Weight (α\alpha) | 13.2 (0.70) | 9.1 (0.49) | 11.8 (0.60) | 12.1 (0.81) | 10.2 (0.91) | 9.1–13.2 |
...
|  |  | Median: 0.31 |  |
| Random Stock Selection | Shuffled signals | Median Sharpe: −0.08-0.08 | p<0.001p<0.001 |
| 50bp Return Noise | Daily Gaussian | Sharpe: 7.8 (from 13.2) | Still unusually high |
| 2×2\times Transaction Costs | 1.2bp total | Sharpe: 9.1 | Remains strong |
...
Liquidity stress represents one of the most challenging environments for quantitative equity strategies, with sharp reductions in market depth, widening bid–ask spreads, and nonlinear impact dynamics amplifying losses for crowded or slow-moving factors.
Historical episodes such as the 2008 liquidity crisis highlight how rapid deleveraging and flight-to-quality dynamics can cause traditional long–short factors to unwind violently, particularly those exposed to funding constraints or liquidity spirals (Frazzini and Pedersen, [2014](https://arxiv.org/html/2511.12490v1#bib.bib11); Hendershott and Menkveld, [2014](https://arxiv.org/html/2511.12490v1#bib.bib13)).
...
Specifically, we apply (i) a 50–70% reduction in displayed depth, (ii) a doubling of effective spreads, (iii) 10bp adverse selection slippage per trade, and (iv) a volatility spike consistent with crisis-regime behavior documented in prior work (Ang and Bekaert, [2002](https://arxiv.org/html/2511.12490v1#bib.bib9)).
Despite these severe assumptions, the strategy maintains an out-of-sample Sharpe ratio of 4.1 (Fig. [4](https://arxiv.org/html/2511.12490v1#S3.F4 "Figure 4 ‣ 3.3 Robustness Validation ‣ 3 Results ‣ Discovery of a 13-Sharpe OOS Factor: Drift Regimes Unlock Hidden Cross-Sectional Predictability")), outperforming both market benchmarks and broad hedge-fund indices, which delivered strongly negative risk-adjusted returns during the same period.

The strategy’s resilience arises from its reliance on short-horizon reversal/value signals that remain operative even when volatility surges, combined with the selective activation mechanism that dramatically reduces exposure when fewer stocks exhibit drift.
...
In addition, the strategy’s market-neutral construction and low correlation to traditional risk factors help decouple returns from systemic deleveraging episodes.
...
| Normal (15≤\leqVIX<<25) | 54% | 38% | 12.1 | 0.93 |
| Stressed (25≤\leqVIX<<35) | 14% | 22% | 7.2 | 0.51 |
| Crisis (VIX≥\geq35) | 4% | 8% | 2.1 | −0.83-0.83 |
...
| --- | --- | --- | --- |
|  |  | Triggered | Triggered |
| Absolute Drawdown | −30%-30\% | 0 of 3 | No |
| Rolling 63-day Loss | −10%-10\% | 0 of 3 | No |
| Volatility Spike | 3×3\times target | 0 of 3 | No |
| Correlation Break | |ρ|>0.5|\rho|>0.5 | 0 of 3 | No |
...
| $2B | 80% | 95 | 0.4 | 4.8% | ×\times Unviable |

Conservative capacity analysis using square-root impact models suggests $100–500 million deployable capital before material performance degradation, with the strategy maintaining double-digit Sharpe ratios below $100 million and acceptable performance up to $500 million (see Fig. [5](https://arxiv.org/html/2511.12490v1#S3.F5 "Figure 5 ‣ 3.5 Attribution and Regime Analysis ‣ 3 Results ‣ Discovery of a 13-Sharpe OOS Factor: Drift Regimes Unlock Hidden Cross-Sectional Predictability")).
...
| Bid-Ask Spread | 100% baseline | 80% | −20%-20\% | Lower costs |
...
* Hendershott and Menkveld (2014)
  ...
  “Price pressures.”
  Journal of Financial Economics, 114(3), 405–423.
